        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // Создаем клиента в таблице Clients
                    var client = new ClientModel
                    {
                        UserId = user.Id,  // Привязываем клиента к пользователю
                        FirstName = Input.Firstname,
                        Email = Input.Email,
                        PhoneNumber = Input.PhoneNumber,
                        Address = Input.PostAddress
                    ...
                    _context.Clients.Add(client);  // Добавляем клиента в базу данных
                    await _context.SaveChangesAsync();  // Сохраняем изменения

                    await _signInManager.SignInAsync(user, isPersistent: false);

                    // Привязываем корзину к новому пользователю
                    await _cartService.CreateOrUpdateCartForUser(user.Id);
